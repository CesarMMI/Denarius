...
rode `.claude/init.sh` e pare se houver falhas.
...
**Se vazio:**
Leia `.claude/feature_list.json` e liste as features com status `pending` ou
`in-progress`, numeradas. Pergunte qual o usuário quer implementar antes de prosseguir.

**Se "next", "nxt", "próximo" ou variações:**
Leia `.claude/feature_list.json` e selecione automaticamente a feature com maior
...
2. Leia o `.claude/claude-progress.md` para entender o estado atual
...
Rode `.claude/init.sh` completo. Só prossiga se tudo passar.
